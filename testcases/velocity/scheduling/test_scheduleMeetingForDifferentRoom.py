...
from pages.velocity.popups.BookingConfirm_popup import BookingConfirmPopup
from pages.velocity.popups.ScheduleMeeting_popup import ScheduleMeetingPopup
...
class TestScheduleMeetingForDiffRoom:
    ...
    def class_setup(self):
        ...
        self.schMeetingPop = ScheduleMeetingPopup(self.driver, self.wait)
        self.bookConfPop = BookingConfirmPopup(self.driver, self.wait)

    def test_scheduleMeetingForDiffRoom(self):
        ...
        for new_window in all_windows:
            if new_window != main_window:
                ...
                self.mRoomControl.clickRoomsIcon()
                time.sleep(1)
                self.mRoomControl.clickRoom2()
                time.sleep(1)
                self.mRoomControl.clickAddBook3()
                time.sleep(1)
                ...
                #Validate booking confirmation popup
                assert self.bookConfPop.visibilityOfOkPop() is True
                self.bookConfPop.clickOk()
